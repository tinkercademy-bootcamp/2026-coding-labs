...
In this assignment, you will implement a custom version of `unique_ptr`. You'll exercise the four topics from this week's videos: **RAII**, **move semantics**, **smart pointers**, and **operator overloading**.

> [!NOTE]
> Before starting, watch:
> - [RAII](https://www.youtube.com/watch?v=YokY6HzLkXs)
> - [Move semantics](https://youtu.be/Rfu06XAhx90)
> - [Operator overloading](https://www.youtube.com/watch?v=i_Z_o9T2fNE)
...
To run your code, first you'll need to compile it. Open up a terminal (if you are using VSCode, hit <kbd>Ctrl+\`</kbd> or go to **Terminal > New Terminal** at the top). Then make sure that you are in the `assignment4-unique-ptr-move-semantics-raii/` directory and run:
...
As you are following the instructions below, we recommend intermittently compiling and running the tests as a way to make sure you're on the right track! Running `./main` with no arguments runs every test in sequence; running `./main <test_name>` runs a single test (the harness will list available names if you pass one it doesn't recognise).
...
> [!IMPORTANT]
> ##### `short_answer.txt`
> **Q5:** Why does `operator->` return `T*` (a pointer) rather than `T&` (a reference)? Walk through what the compiler does when it sees `ptr->member`.
>
> **Q6:** Why does this class need *both* `T& operator*()` and `const T& operator*() const`? Give a one-line code example that would fail to compile if one were missing, and explain which one.
>
> **Q7:** We marked `operator bool()` as `explicit`. What kind of unintended code would compile if we had left the `explicit` off? Give a concrete example.
...
After implementing the above functions, you should be passing all of the tests for **Part 1**.
...
Your `unique_ptr` cannot yet be compared with `==`. We want all three of these expressions to work:

```cpp
auto a = cs106l::make_unique<int>(1);
auto b = cs106l::make_unique<int>(2);
cs106l::unique_ptr<int> c = nullptr;

bool x = (a == b);        // two unique_ptrs
bool y = (a == nullptr);  // unique_ptr vs nullptr
bool z = (nullptr == c);  // nullptr vs unique_ptr (reversed)
```

Two `unique_ptr`s compare equal **iff they hold the same raw pointer** — which, given uniqueness, only happens when both are null. The `nullptr` comparisons should be `true` exactly when the `unique_ptr` is empty.

> [!IMPORTANT]
> ### Implementing `operator==`
>
> Inside the body of `unique_ptr`, declare and define the necessary `operator==` overloads as `friend` functions. There is a TODO block in `unique_ptr.h` showing you where to put them.
>
> - You'll need an overload comparing two `unique_ptr<T>`s.
> - You'll need overloads against `std::nullptr_t` — and in C++20, defining `operator==(unique_ptr, nullptr_t)` is sufficient because the compiler synthesises the reversed `(nullptr, unique_ptr)` form for you. Verify this works on your compiler.
> - You do **not** need to write `operator!=` — C++20 synthesises it from `operator==`.
>
> Think about why these need to be `friend` and could not be ordinary member functions, especially the reversed `nullptr == ptr` form.
...
*Adapted from Stanford CS106L (originally by Jacob Roberts-Baca).*
