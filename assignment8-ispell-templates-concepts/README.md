...
Assuming that your code compiles without any compiler errors, you can run the spellchecker on a string of text:

```sh
./main "This string is mispelled"
./main --stdin < examples/orwell.txt
./main --test                         # run the test suite instead
```

Running `./main` with no arguments prints usage.

As you are following the instructions below, we recommend intermittently compiling and running the tests (`./main --test`) as a way to make sure you're on the right track.
...
1. **Step One: Skip words that are already correctly spelled.**  
    ...
    Either version is fine: `rv::filter` with a namespace alias or `std::ranges::views::filter`.
...
3. **Step Three: Drop misspellings with no suggestions.**  
    ...
    > ```
    > You might be tempted to do something similar here with `std::ranges::to<std::set<Misspelling>>()`. This is a good idea! But the `std::ranges::to` method was only recently introduced in C++23. Depending on the version of compiler you are using, this code may compile or it may not! To be safe, and to make sure that your code compiles across older toolchains, please use the `std::set<Misspelling>` constructor with iterators. **In general, please only use C++ features up through C++20 for this assignment.**
...
So far, every template you've touched has been written the "classic" way: with bare `typename` parameters that accept *anything*. The `find_all` helper you've been calling is a good example.

> 📄 [**`find_all`**](./utils.cpp) (current signature)
> ```cpp
> template <typename Iterator, typename UnaryPred>
> std::vector<Iterator> find_all(Iterator begin, Iterator end, UnaryPred pred);
> ```

Nothing in this signature actually says *"`Iterator` must be an iterator"* or *"`UnaryPred` must be a callable that takes one argument and returns a bool."* If you call `find_all` with the wrong kind of argument — say, you pass an `int` where the predicate goes — the compiler will dutifully try to substitute `int` into the template body, fail deep inside, and dump a screen-long error pointing at the line that tries to invoke `pred(*it)`.

C++20 **concepts** are named, reusable predicates *on types*. You attach them to template parameters to say "this type must satisfy these requirements." If the caller violates the contract, the compiler reports the failure at the call site, in terms of the concept that didn't hold, instead of vomiting up the template's internals.

The standard library already ships a rich set of concepts in `<iterator>` and `<concepts>`. Two we care about here:
...
> ```
>
> Satisfied by any type that can be incremented and dereferenced to read a value — i.e., the minimum interface needed to walk through a range once.

> 📄 [**`std::indirect_unary_predicate`**](https://en.cppreference.com/w/cpp/iterator/indirect_unary_predicate)
> ```cpp
> template <class F, class I>
> concept indirect_unary_predicate = /* ... */;
> ```
>
> Satisfied by any callable `F` that can be invoked on the value pointed to by an iterator `I` and returns something contextually convertible to `bool`. This is exactly the contract `find_all` wants from its predicate.

Your task is to constrain `find_all` using these concepts. You will need to update **two** places:

1. The forward declaration in `spellcheck.cpp` (near the top).
2. The definition in `utils.cpp`.

Replace the `typename Iterator, typename UnaryPred` parameter list with the equivalent concept-constrained form. Don't forget to `#include <iterator>` and `#include <concepts>` if they aren't already pulled in transitively.

> 💡 **Hint**
> The constrained signature looks like:
> ```cpp
> template <std::input_iterator Iterator,
>           std::indirect_unary_predicate<Iterator> UnaryPred>
> std::vector<Iterator> find_all(Iterator begin, Iterator end, UnaryPred pred);
> ```
> Notice how `UnaryPred` is constrained *in terms of* `Iterator` — the predicate must be callable on whatever `Iterator` dereferences to.

Once you've made the change, your existing `tokenize` should still compile and pass tests exactly as before — concepts don't change runtime behavior, just what the compiler will accept.

### Observing the payoff

To see what concepts actually buy you, temporarily break a call to `find_all` in your `tokenize`. For example, swap the `::isspace` predicate for a plain integer:

```cpp
auto its = find_all(source.begin(), source.end(), 42);  // 42 is not a predicate!
```

Compile and read the error message. With the old unconstrained template, the error points somewhere inside `find_all` where `pred(*it)` is called. With the constrained version, the compiler tells you up front that the call to `find_all` failed because `42` doesn't satisfy `std::indirect_unary_predicate`. Much friendlier!

Once you've seen the difference, restore your original `tokenize` call.
...
./main --test <name>          # run one named test
```
...
* `spellcheck.cpp`
* `utils.cpp` (only the `find_all` signature should change)

*Adapted from Stanford CS106L Assignment 4.*
